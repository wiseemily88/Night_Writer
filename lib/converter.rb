...
class Converter
  ...
  def braille_or_english_path
    if is_braille?
      match_braille_to_english(message_input)
    else
      match_english_to_braille(message_input)
    end
  end
  ...
  def match_english_to_braille(message_input)

    message_input.map do |letter|
      new_dictionary = Dictionary.new
      translated_braile = new_dictionary.english_in_to_braille[letter]
  ...
  def match_braille_to_english(message_input)
    ...
    message_input.map do |letter|
      new_dictionary = Dictionary.new
      translated_braile = new_dictionary.braille_to_english[letter]
      split_lines
    end
  end
